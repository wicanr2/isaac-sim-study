...
| 35–39 | HIL:STM32(Renode)下位控制器 + Rust 橋接 + Isaac 受控體;39 為 FreeRTOS 版 | `docs/hil/` + `examples/hil-stm32/` | 第一版完成(2026-09-15);Isaac 6.0.1 側同日實測;Renode 四項修正在 fork;GOAL 3 四線完成(2026-09-16,上游 PR #250、renode#1003/#1004);GOAL 4 四線完成(同日:encoder mode、斜坡與馬達層、安全 I/O 五項、Nav2 in the loop;issue #2–#5);GOAL 5 五線完成(2026-09-17:閒時量測、兩版韌體去重、Isaac 側補齊、上位對安全旗標、俯視圖錄影;#6;#2–#6 已關,encoder-wrap 送 renode-infrastructure PR #252);GOAL 6(`docs/goal/GOAL-2026-09-17-1240.md`,2026-09-17)三線完成、一線部分:打滑偵測(#8,上游 PR #254)、IWDGRSTF 與 AMCL 重定位(#9,上游 PR #253)、Isaac 側次要項(#10)已關;realtime 連續注入(#7)閒時 C9 3/5,未達五次全綠,開放;GOAL 7(`docs/goal/GOAL-2026-09-17-1700.md`,IMU 優先)三線完成:誤差模型與零偏估計 #11、加速度計補平移打滑 #12(Renode 加速度計模型修正待送上游)、gyrodometry 航向融合 C15 #13 已關;C14 高負載不穩(時鐘)與打滑物理化、牽引力控制、上位脫困、預防碰撞轉 GOAL 8(`docs/goal/GOAL-2026-09-18-0015.md`);realtime 抖動 #7 開放;決定與結果在 issue #1;歷輪 GOAL 檔見 `docs/goal/index.md` |
