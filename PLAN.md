...
| 35–39 | HIL:STM32(Renode)下位控制器 + Rust 橋接 + Isaac 受控體;39 為 FreeRTOS 版 | `docs/hil/` + `examples/hil-stm32/` | 第一版完成(2026-09-15);Isaac 6.0.1 側同日實測;Renode 四項修正在 fork;GOAL 3 四線完成(2026-09-16,上游 PR #250、renode#1003/#1004);GOAL 4 四線完成(同日:encoder mode、斜坡與馬達層、安全 I/O 五項、Nav2 in the loop;issue #2–#5);GOAL 5 五線完成(2026-09-17:閒時量測、兩版韌體去重、Isaac 側補齊、上位對安全旗標、俯視圖錄影;#6;#2–#6 已關,encoder-wrap 送 renode-infrastructure PR #252);下一階段 `docs/goal/GOAL-2026-09-17-1240.md`(realtime 連續注入、打滑偵測、IWDGRSTF 與 AMCL 重定位、Isaac 側次要項;#7–#10);決定與結果在 issue #1;歷輪 GOAL 檔見 `docs/goal/index.md` |
