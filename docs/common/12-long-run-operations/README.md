...
| 3 | 狀態漂移(見 [11 篇 §9](../../5.1/11-live-pose-and-accuracy/README.md)) | 只跑狀態重置 | 不碰任何程序 |
...
我們曾在「自己的診斷腳本已經把模擬弄壞了」的狀態下(就是 [11 篇 §3](../../5.1/11-live-pose-and-accuracy/README.md) 那個 simulation view 失效事故),去測兩個啟動旗標對機構動作的影響。結論是「這兩個旗標會讓機構失效」——冤枉了寫那段程式的同事。重測四種組合才平反:四種組合全部正常。
...
- [ ] 重置流程結尾重新套用執行期物理設定(見 [10 篇 §4](../../common/10-scene-physics-authoring/README.md))
