...
**再強調一次 [04 篇](../../common/04-physics-world/README.md)的鐵則**:模擬不在 PLAYING,TF 與 `/joint_states` 都不發布。外部系統看到的症狀是「pose 無效、topic 空的、機器人離線」,第一步永遠先查模擬狀態。
...
注意第一條的教訓與 [01 篇](../../common/01-install-and-run-modes/README.md)的 Python 隔離原則同源:Isaac 內建 DDS 與系統 ROS2 的 discovery 實測未能互通,根因指向兩邊 DDS 實作版本與 discovery 設定的差異——理論上可以對齊,但實戰成本高且脆。「在 Isaac 裡面跑一個 ROS 節點」聽起來最直接,實際上是最不穩的一條路。
...
| 命令送達但車不動 | joint drive 增益、teleport vs target 語意(見 [04 篇](../../common/04-physics-world/README.md) §4) |
...
- 下一篇:[06 WebRTC 串流與多 client](../../common/06-webrtc-streaming/README.md)
