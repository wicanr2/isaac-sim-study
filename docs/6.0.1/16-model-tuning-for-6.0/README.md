...
延伸閱讀:[13 接觸與抓握的第一性原理](../../common/13-contact-and-grasp-first-principles/README.md)(為什麼調摩擦常是錯的第一步)、[10 場景資產的物理結構](../../common/10-scene-physics-authoring/README.md)、[15 5.1 → 6.0 的物理層變動](../../6.0.1/15-physics-backend-5.1-to-6.0/README.md)。
...
摩擦是**兩個**接觸面材質合成的結果,合成方式由 `physxMaterial:frictionCombineMode` 決定:

| combine mode | 合成方式 | 單側綁 μ=5.0、另一側預設 0.5 的結果 |
|---|---|---|
| `average`(**schema 預設**) | 取平均 | 2.75 —— 被稀釋掉一半 |
| `min` | 取小 | 0.5 —— **綁了等於沒綁** |
| `max` | 取大 | **5.0 —— 單側綁定就足夠** |
| `multiply` | 相乘 | 2.5 |
...
**但沒綁的那一側仍然有影響**:凡是**不涉及叉齒**的接觸(棧板 ↔ 載具、棧板 ↔ 地面、棧板 ↔ 貨架),兩側都沒有物理材質,全部走 PhysX 預設。

> **教訓**:看到「單側沒綁物理材質」時,不要直接下結論說摩擦沒生效。**先讀 `frictionCombineMode`。** 在 `max` 之下單側綁定就夠,在 `min` 之下綁了等於沒綁 —— 同一個綁定狀態,四種 mode 給出四種完全不同的結論。
...
要讓叉齒插進棧板的叉孔,碰撞近似必須**表達得出那個孔**。凸包(`convexHull`)在定義上做不到——孔內每一點都是孔周頂點的凸組合,必然被填實(推導見 [13 篇](../../common/13-contact-and-grasp-first-principles/README.md))。所以要用 `sdf`。
