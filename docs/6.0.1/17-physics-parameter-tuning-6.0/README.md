...
前一篇([16](../../6.0.1/16-model-tuning-for-6.0/README.md))講的是**結構**——貼紙貼在哪一層。這一篇講**數值**:6.0 有哪些物理參數可以調、從哪裡調、以及一個參數要滿足什麼條件才會真的生效。
...
延伸閱讀:[16 模型結構調校](../../6.0.1/16-model-tuning-for-6.0/README.md)、[15 物理層變動](../../6.0.1/15-physics-backend-5.1-to-6.0/README.md)、[09 物理模擬基礎](../../common/09-physics-simulation-fundamentals/README.md)。
...
**物理參數的絕大多數是第一種。** 第二種主要管全域行為與 UI 預設,第三種是啟動腳本 patch 的做法(見 [16 篇 §3.2](../../6.0.1/16-model-tuning-for-6.0/README.md))。
...
6.0 有 PhysX 與 Newton 兩個後端([15 篇](../../6.0.1/15-physics-backend-5.1-to-6.0/README.md))。`physx*` 命名空間的屬性由 PhysX 讀,`newton:*` 由 Newton 讀。**設了另一個後端的屬性,會被靜默忽略。**
...
| 屬性 | 預設 | 說明 |
|---|---|---|
| `sdfResolution` | **256** | **體素間距 = 最長 AABB 邊 ÷ 此值**(見 [16 篇 §4.4](../../6.0.1/16-model-tuning-for-6.0/README.md)) |
...
| **`physxJoint:maxJointVelocity`** | **`inf`** | **關節速度上限。5.x 時代預設是 1e6,6.0 拿掉了**(見 [15 篇 §4.5](../../6.0.1/15-physics-backend-5.1-to-6.0/README.md)) |
...
- [13 接觸與抓握的第一性原理](../../common/13-contact-and-grasp-first-principles/README.md) — 為什麼調參順序是幾何優先
- [15 物理層變動](../../6.0.1/15-physics-backend-5.1-to-6.0/README.md) §4.5 — `maxJointVelocity` 在 107 → 110 的變更
