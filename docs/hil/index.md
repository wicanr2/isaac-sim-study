...
| [38](38-acceptance-and-failure-modes/README.md) | 驗收與失敗形態 | 八項判準在開跑前寫死;負對照在 C2 轉紅而 C6 證明韌體擋了全部 300 個壞框包;兩次 CSV 逐 byte 相同;**步邊界取樣的 2/305 不符**與事件時刻快照;十三種失敗形態;換成 Isaac 6.0.1 的七件事各量到什麼——**PhysX 介面沒有 `update`、joint state 不寫回、地面 xformOpOrder 反了讓車飛起來**、滑移 3.1%;ROS 2 方形閉環的閉合誤差拆成轉角過頭與起步偏航 |
