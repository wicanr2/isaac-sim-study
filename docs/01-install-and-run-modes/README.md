...
Isaac Sim 常被當成「一個要開視窗操作的 3D 軟體」,這個印象只對了三分之一。它的本體是 Omniverse Kit 應用程式,視窗(GUI)只是其中一種前端;同一套模擬核心可以完全不開視窗跑在遠端伺服器上,畫面經串流送出、指令經腳本送入。要走到「純 Python 遠端操作」,第一步是弄清楚它有哪幾種執行模式、各自解決什麼問題。
...
- **Shader cache 要放進 named volume**。首次啟動要冷編譯 shader(數分鐘),cache 掛 volume 之後暖啟動可以縮短到約一半。
...
Isaac Sim 的 RTX 渲染層(OptiX 外掛)是針對特定範圍的驅動 ABI 編譯的,**版本與驅動不匹配時不是效能下降,是直接 segfault**。實戰踩過的組合:
...
- **資料中心卡(L40S、A100 等)預設開 ECC,是 RTX 渲染間歇 crash 的已知因子**。`sudo nvidia-smi -e 0` 關閉後重開機。
- **CPU / RAM 不能省**:首次 shader 編譯會吃滿所有核心,8 vCPU 的機器可能連 SSH 都餓死。實戰結論是 Isaac Sim 主機至少 16 vCPU / 64 GB RAM。
