// Runtime-loadable copy of upstream master STM32_Timer.cs (renode-infrastructure 47a4e12, 2026-09-09), class renamed to
// STM32_Timer_Master so it can be loaded into Renode 1.16.1 with `i @file.cs`. It has encoder mode (SMS=001/010/011)
// and input capture, which 1.16.1 lacks. It does NOT have the three fixes of STM32_Timer.patch (issue renode#1003).
//
...
namespace Antmicro.Renode.Peripherals.Timers
{
    ...
    public class STM32_Timer_Master : LimitTimer, IDoubleWordPeripheral, IKnownSize, INumberedGPIOOutput, IRegisterablePeripheral<IGPIOReceiver, NumberRegistrationPoint<int>>, IRegisterablePeripheral<IGPIOReceiver, NullRegistrationPoint>, IGPIOReceiver
    {
        public STM32_Timer_Master(IMachine machine, ulong frequency, uint initialLimit) : base(machine.ClockSource, frequency, limit: initialLimit, direction: Direction.Ascending, enabled: false, eventEnabled: true, autoUpdate: false)
        {
        ...
        {
            public CaptureCompareChannel(STM32_Timer_Master parent, int index)
            {
                this.parent = parent;
                Index = index;
                Connection = new GPIO();
            ...
            private readonly STM32_Timer_Master parent;
        }
