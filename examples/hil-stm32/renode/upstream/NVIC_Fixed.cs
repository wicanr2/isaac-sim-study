//
// NVIC with one fix over Renode 1.16.1 (renode-infrastructure add012af0):
//   SysTick loads RELOAD when ENABLE goes 0->1 (ARMv7-M B3.3.3). Without it, the FreeRTOS
//   port write order (CVR=0, RVR=n, CSR=7) starts the first period at 0xFFFFFF = 233 ms @ 72 MHz.
// Class renamed for runtime loading / NUnit against 1.16.1 binaries; the CPU model type-checks
// its nvic argument, so this copy is for the unit test only, not for a platform description.
//
//
...
namespace Antmicro.Renode.Peripherals.IRQControllers
{
    ...
    public class NVIC_Fixed : IDoubleWordPeripheral, IHasDivisibleFrequency, IKnownSize, IIRQController, IHasMappedRegisters
    {
        public NVIC_Fixed(IMachine machine, ulong systickFrequency = 50 * 0x800000, byte priorityMask = 0xFF, bool haltSystickOnDeepSleep = true)
        {
        ...
        {
            public SysTick(IMachine machine, NVIC_Fixed parent, ulong systickFrequency, bool isSecure = false)
            {
                IsSecure = isSecure;
                this.parent = parent;
            ...
            private readonly NVIC_Fixed parent;
        }
