//
// STM32_Timer with three fixes over Renode 1.16.1 (renode-infrastructure add012af0):
//   1. counter period is ARR+1 ticks, not ARR (RM0090 18.3.1)
//   2. OCxPE preload: CCRx writes are transferred on the next update event (RM0090 18.4.7)
//   3. PWM outputs are driven on CEN and on UG, not only after the first overflow
// Loaded at runtime with `i @STM32_Timer_Fixed.cs`; the platform description uses
// Timers.STM32_Timer_Fixed in place of Timers.STM32_Timer. Same source with the class
// renamed back is what goes upstream (see docs/hil/37 section 4).
//
//
...
namespace Antmicro.Renode.Peripherals.Timers
{
    ...
    public class STM32_Timer_Fixed : LimitTimer, IDoubleWordPeripheral, IKnownSize, INumberedGPIOOutput, IRegisterablePeripheral<IGPIOReceiver, NumberRegistrationPoint<int>>, IRegisterablePeripheral<IGPIOReceiver, NullRegistrationPoint>, IGPIOReceiver
    {
        public STM32_Timer_Fixed(IMachine machine, ulong frequency, uint initialLimit) : base(machine.ClockSource, frequency, limit: initialLimit, direction: Direction.Ascending, enabled: false, eventEnabled: true, autoUpdate: false)
        {
