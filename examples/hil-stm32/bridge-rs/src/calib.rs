...
pub struct Calib {
    ...
    pub motor_free_rad_s: f64,
    pub motor_torque_max_nm: f64,
    pub friction_mu: f64,
    /// 正規化庫侖摩擦的滑移參考速度(m/s):|滑移| 小於它時摩擦力線性,超過就飽和在 μN
    pub friction_v_ref_m_s: f64,
    pub robot_mass_kg: f64,
    pub wheel_mass_kg: f64,
    pub body_inertia_kg_m2: f64,
...
impl Calib {
    pub fn load(path: &str) -> io::Result<Calib> {
        ...
        Ok(Calib {
            ...
            motor_torque_max_nm: num(&t, "motor_torque_max_nm").unwrap_or(2.45),
            friction_mu: num(&t, "friction_mu").unwrap_or(0.508),
            friction_v_ref_m_s: num(&t, "friction_v_ref_m_s").unwrap_or(0.02),
            robot_mass_kg: num(&t, "robot_mass_kg").unwrap_or(9.2),
            wheel_mass_kg: num(&t, "wheel_mass_kg").unwrap_or(0.5),
            body_inertia_kg_m2: num(&t, "body_inertia_kg_m2").unwrap_or(0.11),
            wheel_radius_mm: r,
